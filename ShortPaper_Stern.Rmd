...
```{r, fig.width = 10, fig.height= 10, warning= FALSE, results='hide',fig.keep='all', message=FALSE}
...
barplot_leg <- ggplotGrob(ggplot(leg_data, aes(x=fund_type, y=total_funding_mil)) +
                  ...
                  theme(
                    ...
                    axis.title.x = element_text(family = "Montserrat", color = "white"),
...
ggplot() +
  ...
  annotate(geom = "label", x = -2.235833, y = 8.18, label = "Bui Dam", 
    fontface = "bold", color = "#E31480", size = 4, family = "Montserrat") +
  scale_colour_manual(values = c("#C42231", "#F69D30", "#2699D0", "#4CA145")) + 
  coord_sf(crs = 4326, datum = NA) +
  map_th +
  labs(x = NULL, y = NULL,
  ...
  annotation_custom(
    grob = barplot_leg,
    xmin = -0.4, xmax = 1.2, ymin = 4.5, ymax = 5.3 ) #+
